...
def write_markdown_report(payload: dict[str, Any], path: Path) -> None:
    lines: list[str] = []
    lines.append("# Spike 单 case 覆盖率矩阵")
    lines.append("")
    lines.append("## 结论")
    lines.append("")
    ...
    lines.append(f"- Runner 状态: {status_text}。")
    lines.append(f"- 覆盖率 counter movement: changed counters={changed}，still-zero events={still_zero}。")
    lines.append(
        f"- 正向证据候选：PASS 且 counter movement 的 case={len(payload.get('pass_cases_with_counter_changes', []))}；"
        f"非 PASS 但 counter movement 的 case={len(payload.get('nonpass_cases_with_counter_changes', []))}；"
        f"无执行证据 case={len(payload.get('cases_with_no_execution_evidence', []))}；"
        f"scope-warning case={len(payload.get('cases_with_scope_warnings', []))}。"
    )
    lines.append("- 本报告只能作为证据；最终缺失场景/测试点结论仍需要源码复核和 target-scope 检查。")
    lines.append("")
    lines.append("## 数据")
    lines.append("")
    ...
    lines.append("## 限制与下一步")
    lines.append("")
    lines.append("- 单 case counter movement 是证据，不是最终 path proof。")
    lines.append("- 只有 PASS 且 in-scope 的 counter movement 可以作为正向覆盖线索；非 PASS movement 只能诊断。")
    lines.append("- 写测试点卡片前要检查单 case `compare.md`、run log 和 Spike 源码。")
    lines.append("- 需要证明同一条指令/访问流时，请使用 requirements 或 path marker。")
    lines.append("")
    lines.append("## 证据")
    lines.append("")
    lines.append("## 输入")
    ...
    if payload.get("unresolved_gcno_tokens"):
        lines.append("## 未解析的 gcno hint")
        lines.append("")
        lines.append(
            "这些 target hint 没有解析到独立 `.gcno`。"
            "header `.gcov` 仍可能由另一个已选择 object 生成，例如 `mmu.gcno`。"
        )
        lines.append("")
    ...
    lines.append("## 已选择 case")
    lines.append("")
    if payload.get("selected_cases"):
        lines.append("| # | Case | ELF | 建议 |")
        lines.append("|---:|---|---|---|")
    ...
    if missing_elf_results:
        lines.append("## Missing ELF 诊断")
        lines.append("")
        lines.append(
            "这些 case 没有执行 Spike，属于选择/环境配置问题，不是覆盖率证据。"
            "用于路径分析前，需要先修正 case 名、artifact 映射或 ELF 路径。"
        )
        lines.append("")
    ...
    if payload.get("cases_with_missing_target_gcda"):
        lines.append("## 运行后缺少 target gcda 的 case")
        lines.append("")
        lines.append(
            "这些 PASS case 已执行 Spike，但至少一个选中的 target object 由缺失 `.gcda` 生成 `.gcov`。"
            "这通常说明该 case 没有执行那个 Spike object，或选择的 case 不在目标分析范围内。"
        )
        lines.append("")
        for result in payload.get("results", []):
    ...
    if payload.get("nonpass_cases_with_counter_changes"):
        lines.append("## 非 PASS 但 counter 有 movement 的 case")
        lines.append("")
        lines.append(
            "这些 case 移动了覆盖率 counter，但没有通过 runner marker。"
            "它们的 delta 只能作为诊断线索，不能作为正向 same-flow/path 证明。"
        )
        lines.append("")
        for result in payload.get("results", []):
    ...
    if payload.get("cases_with_scope_warnings"):
        lines.append("## Target scope 警告")
        lines.append("")
        lines.append(
            "这些 case 通过了 runner，但日志命中了 target `scope_out` 语言。"
            "确认 target/profile 决策前，不要把它们当作 in-scope 正向证据。"
        )
        lines.append("")
        for result in payload.get("results", []):
            runner = result.get("runner") or {}
    ...
    if non_pass_results:
        lines.append("## Runner 问题")
        lines.append("")
        lines.append(
            "这些只是 runner/log 问题。覆盖率 counter 可能仍会移动，"
            "但在理解日志问题前，不要把失败 case 用作正向路径证明。"
        )
        lines.append("")
    ...
    if payload.get("results") and not payload.get("skip_gcov"):
        lines.append("## 单 case 覆盖率亮点")
        lines.append("")
        lines.append(
            "| # | Case | 证据用途 | 新增覆盖/计数增加的 top 文件 | Entry/header movement | 仍为 0 的 entry/header | 仍为 0 的 top 文件 |"
        )
        lines.append("|---:|---|---|---|---|---|---|")
        for result in payload.get("results", []):
            lines.append(
    ...
    lines.append("## 单 case 矩阵")
    lines.append("")
    lines.append(
        "| # | Case | Runner | 证据用途 | 必需证据通过 | Counter changes | Missing gcda | 无效证据 | Compare | Run log |"
    )
    lines.append("|---:|---|---|---|---|---:|---:|---:|---|---|")
    for result in payload.get("results", []):
